...
export default class ESService {
  ...
  executeSearch = async (
    ...
    opensearchDashboardsResponce: OpenSearchDashboardsResponseFactory
    ...
    try {
      ...
      return opensearchDashboardsResponce.ok({ body: { ok: true, response: results } });
    } catch (err) {
      console.error('Anomaly detector - Unable to execute search', err);
      return opensearchDashboardsResponce.ok({
        body: {
          ok: false,
  ...
  bulk = async (
    ...
    try {
      ...
      return opensearchDashboardsResponce.ok({ body: { ok: true, response: { response } } });
    } catch (err) {
